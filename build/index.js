...
MenuItem.propTypes = { label: _propTypes2.default.string.isRequired, className: _propTypes2.default.string };
...
var _fuse = __webpack_require__(338);

var _fuse2 = _interopRequireDefault(_fuse);

var _MenuSection = __webpack_require__(339);
...
var QuickSelectMenu = function (_Component) {
  _inherits(QuickSelectMenu, _Component);

  function QuickSelectMenu(props) {
    _classCallCheck(this, QuickSelectMenu);

    var _this = _possibleConstructorReturn(this, (QuickSelectMenu.__proto__ || Object.getPrototypeOf(QuickSelectMenu)).call(this, props));

    _initialiseProps.call(_this);

    var menuSections = _this.props.menuSections;


    var filteredItemsList = _this.createFilteredItemsList(menuSections);
    var sectionsByPrefix = _this.createSectionsByPrefix(menuSections);
    var filteredSections = sectionsByPrefix[''] ? sectionsByPrefix[''] : menuSections;

    _this.state = {
      filteredSections: filteredSections,
      filteredItemsList: filteredItemsList,
      sectionsByPrefix: sectionsByPrefix,
      activeItemIndex: 0
    };
    return _this;
  }

  _createClass(QuickSelectMenu, [{
    ...
    value: function render() {
      ...
      for (var i = 0; i < filteredSections.length; i += 1) {
        itemsCounted += filteredSections[i].items.length;
        if (itemsCounted > activeItemIndex) break;
        activeSectionIndex += 1;
      }

      var sections = filteredSections.map(function (section, i) {
        var label = section.label,
            items = section.items;

        if (items.length === 0) return;

        var activeIndex = activeSectionIndex === i ? activeItemIndex - (itemsCounted - items.length) : null;
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.createFilteredItemsList = function (sections) {
    return sections.reduce(function (acc, _ref) {
  ...
  this.createSectionsByPrefix = function (sections) {
    return sections.reduce(function (acc, section) {
      var _section$prefix = section.prefix,
          prefix = _section$prefix === undefined ? '' : _section$prefix;

      if (acc[prefix]) acc[prefix].push(section);else acc[prefix] = [section];
      return acc;
    }, {});
  };
  ...
  this.handleKeyDown = function (event) {
    ...
    if (key === ENTER) _this3.selectItemByIndex(_this3.state.activeItemIndex);
  };

  this.getItemIndex = function (item) {
    var filteredItemsList = _this3.state.filteredItemsList;

    var indexOfItem = filteredItemsList.indexOf(item);
  ...
  this.setFilteredSections = function (sections) {
    ...
    _this3.setState({
      ...
      filteredItemsList: _this3.createFilteredItemsList(sections),
      activeItemIndex: 0
    });
  };

  this.setActiveItem = function (item) {
    return _this3.setActiveItemByIndex(_this3.getItemIndex(item));
  };

  this.selectItem = function (item) {
    return _this3.selectItemByIndex(_this3.getItemIndex(item));
  };
  ...
  this.filterSections = function (value) {
    if (typeof value !== 'string') {
      throw new TypeError('Invalid argument \'value\'. Must be of type string.');
    }

    var menuSections = _this3.props.menuSections;
    var sectionsByPrefix = _this3.state.sectionsByPrefix;


    if (value === '') {
      var _filteredSections = sectionsByPrefix[''] ? sectionsByPrefix[''] : menuSections;
      var cappedFilteredSections = _this3.itemCapSections(_filteredSections);
      return _this3.setFilteredSections(cappedFilteredSections);
    }

    var prefix = Object.keys(sectionsByPrefix).find(function (prefix) {
      return prefix !== '' && prefix === value.slice(0, prefix.length);
    });

    var prefixFilteredMenuSections = prefix ? sectionsByPrefix[prefix] : sectionsByPrefix[''];
    ...
    var filteredSections = prefixFilteredMenuSections.reduce(function (acc, section) {
      ...
      if (prefixTrimmedValue.length > 0) {
        var fuse = new _fuse2.default(section.items, options);
        items = fuse.search(prefixTrimmedValue);
      }

      acc.push(_extends({}, section, { items: items }));
      return acc;
    }, []);
  ...
  this.itemCapSections = function (sections) {
    var _props$maxItemsToDisp = _this3.props.maxItemsToDisplay,
        maxItemsToDisplay = _props$maxItemsToDisp === undefined ? Infinity : _props$maxItemsToDisp;


    return sections.reduce(function (_ref3, section) {
      var itemsEncountered = _ref3.itemsEncountered,
          acc = _ref3.acc;

      var amountOfItemsThatCanBeAddedToMenu = maxItemsToDisplay - itemsEncountered;
      if (amountOfItemsThatCanBeAddedToMenu <= 0) return { acc: acc, itemsEncountered: itemsEncountered };

      var items = section.items.slice(0, amountOfItemsThatCanBeAddedToMenu);

      acc.push(_extends({}, section, { items: items }));
      return { acc: acc, itemsEncountered: itemsEncountered + items.length };
    }, { acc: [], itemsEncountered: 0 }).acc;
  };
  ...
  this.removeFilters = function () {
    ...
    _this3.setState({
      ...
      filteredItemsList: _this3.createFilteredItemsList(menuSections)
    });
  };

  this.moveUp = function () {
    var _state3 = _this3.state,
        filteredItemsList = _state3.filteredItemsList,
        activeItemIndex = _state3.activeItemIndex;

    var nextIndex = activeItemIndex === 0 ? filteredItemsList.length - 1 : activeItemIndex - 1;
    _this3.setActiveItemByIndex(nextIndex);
  };

  this.moveDown = function () {
    var _state4 = _this3.state,
        filteredItemsList = _state4.filteredItemsList,
        activeItemIndex = _state4.activeItemIndex;

    var nextIndex = activeItemIndex === filteredItemsList.length - 1 ? 0 : activeItemIndex + 1;
    _this3.setActiveItemByIndex(nextIndex);
  };
};

exports.default = QuickSelectMenu;
...
var MenuSection = function (_Component) {
